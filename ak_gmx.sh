...
do 
	if [ -f "$var" ]
       then 
          ...
          wget  http://www.mdtutorials.com/gmx/lysozyme/Files/$var
  fi
done
...
tmp3=_processed.gro
tmp4=$mod_pdb$tmp3
echo 6 | gmx pdb2gmx -f $tmp2 -o $tmp4 -water spce
...
gmx grompp -f ions.mdp -c $tmp8 -p topol.top -o ions.tpr
...
echo 18 0 | gmx energy -f npt_$mod_pdb.edr -o $analysis/pressure_$mod_pdb.xvg
xmgrace $analysis/pressure_$mod_pdb.xvg &

echo 24 0 | gmx energy -f npt_$mod_pdb.edr -o $analysis/density_$mod_pdb.xvg
...
gmx mdrun -v -deffnm md_01_$mod_pdb -gpu_id 0 -ntmpi 1 -ntomp 8 -cpi md_01_$mod_pdb.cpt 
...
echo 4 | gmx mindist -f md_01_noPBC_$mod_pdb.xtc -s md_01_$mod_pdb.tpr -od $analysis/minimal-periodic-distance_$mod_pdb.xvg -pi
